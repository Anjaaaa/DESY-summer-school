...
for filedate in filedates:
    ### Read data
    timestamp_pt100, T_amb, V_left, V_right, I_left, I_right, H, T_chill, pt100_PL, pt100_PR, pt100_TL, pt100_TR = np.genfromtxt(filedate + '.txt', unpack = True)
    timestamp_camera, Tcam_PL, Tcam_PR, Tcam_TL, Tcam_TR, Pcam_PL, Pcam_PR, Pcam_TL, Pcam_TR = np.genfromtxt(filedate + '_Camera.txt', unpack = True)
    ...
    T_eps95_TL, T_eps95_TR, T_eps95_PL, T_eps95_PR = np.genfromtxt(filedate + '_eps095.txt', unpack = True)
    ### EMISSIVITY
    eps_paint = '092'
    for k in range(0,4):
        if (filedate == '180808'):
            time, T_eps98_TL, T_eps98_TR, T_eps98_PL, T_eps98_PR = np.genfromtxt(filedate + '_eps' + eps_paint + '.txt', unpack = True)
